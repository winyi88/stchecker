...
def get_robust_roe(ticker, tk_obj):
    ...
    # Method 2: Yahoo 台灣 HTML 暴力備援 (免疫 API 封鎖)
...
# 頁面配置與自訂 CSS
# ==========================================
st.set_page_config(page_title="全方位個股掃描系統 V8.11", layout="wide", page_icon="🏦")
...
def fetch_data(ticker):
    ...
    with redirect_stdout(f), redirect_stderr(f):
        ...
        # 【V8.11 修復】完全移除破壞 yfinance 雲端防護機制的 Session 注入
...
# UI 介面繪製
# ==========================================
st.title("🏦 全方位個股掃描系統 V8.11 (QuantSight)")
render_html(
        ...
        if not data:
            ...
            with col_t3_right:
                ...
                <div class='pro-card card-pa'>
                    <div class='pro-title'>🕯️ 裸 K 價格行為 (V8.11 語義學診斷)</div>
            ...
            with col_t2:
                st.download_button(label="📥 下載 CSV 數據", data=csv_bytes,
                                   file_name=f"{stock_name}_{clean_ticker}_V8.11_Analysis.csv", mime="text/csv",
                                   width="stretch")
